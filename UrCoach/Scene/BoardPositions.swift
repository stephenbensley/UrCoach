...
final class BoardPositions {
    ...
    public static let indexOffset = Ur.pieceCount + 1
    ...
    init() {
        ...
        for i in -1...Ur.spaceCount {
            positions[0].append(Self.whiteSquarePosition(i))
        }
        positions[1] = positions[0].map({ $0.reflectedOverY })
    }
    
    // Returns the position based on index, where zero is the entry square.
    func position(_ player: PlayerColor, _ index: Int) -> CGPoint {
        // Zero square has Ur.pieceCount waiting positions and the 'just before entry' square
        // ahead of it in the positions array.
        positions[player.rawValue][index + Self.indexOffset]
    }
    ...
    // after exit.
    static func whiteSquarePosition(_ index: Int) -> CGPoint {
        switch index {
        case -1...3:
            return squarePosition(.init(x: -1, y: -index))
        case 4...11:
            return squarePosition(.init(x: 0, y: index - 7))
        case 12...14:
            return squarePosition(.init(x: -1, y: 16 - index))
        default:
            return .init()
        }
    }
    ...
    // rosette is (0, 1).
    static func squarePosition(_ coordinates: CGPoint) -> CGPoint {
        .init(
